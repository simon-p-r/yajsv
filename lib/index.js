...
internals.Manager.prototype.load = function (type, name, rawSchema) {
...
internals.Manager.prototype.filterSchemas = function () {
    ...
    for (var i = 0, il = keys.length; i < il; ++i) {
        var name = keys[i];
        var rawSchema = this.schemas[name];
...
internals.Manager.prototype.find = function (match) {
    ...
    for (var i = 0, il = keys.length; i < il; ++i) {
        var name = keys[i];
        if (match === name) {
            retVal = this.schemas[name].schema;
        }
    };
    return retVal;
};
...
internals.Manager.prototype.getValidator = function () {

    return internals.validator;
};
